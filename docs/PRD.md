...
(관리자모드는 기능은 구현되어 있으나 메인 메뉴에서 비활성화됨 — 아래 "8. 관리자모드" 절 참고)
...
### 8. 관리자모드 (현재 메인 메뉴에서 비활성화됨)

테스트/시연을 빠르게 진행하기 위한 관리자 전용 메뉴로 설계·구현되었다. 다만 실제 사용자에게 노출될 필요가 없다는 판단에 따라, 현재는 메인 메뉴 목록에 표시되지 않고 번호(8)를 입력해도 선택되지 않도록 라우팅을 비활성화한 상태다 (`AdminController` 자체는 그대로 남아 있어 필요시 라우팅만 복원하면 재사용 가능).
